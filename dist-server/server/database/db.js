...
import { APP_CONFIG_TABLE_SQL, USER_NOTIFICATION_PREFERENCES_TABLE_SQL, VAPID_KEYS_TABLE_SQL, PUSH_SUBSCRIPTIONS_TABLE_SQL, SESSION_NAMES_TABLE_SQL, SESSION_NAMES_LOOKUP_INDEX_SQL, DATABASE_SCHEMA_SQL } from './schema.js';
...
const deviceDb = {
    upsert: (id, userId, name, platform) => {
        db.prepare(`
      INSERT INTO devices (id, user_id, name, platform, last_seen, is_online)
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        is_online = 1
    `).run(id, userId, name, platform);
        return db.prepare('SELECT * FROM devices WHERE id = ?').get(id);
    },
    setOnline: (id, isOnline) => {
        db.prepare('UPDATE devices SET is_online = ?, last_seen = CURRENT_TIMESTAMP WHERE id = ?')
            .run(isOnline ? 1 : 0, id);
    },
    getByUser: (userId) => {
    ...
    markAllOffline: (userId) => {
        db.prepare('UPDATE devices SET is_online = 0 WHERE user_id = ?').run(userId);
    }
};
// Pair code database functions
const pairCodeDb = {
    create: (code, userId, ttlSeconds = 300) => {
        db.prepare(`
...
export { db, initializeDatabase, userDb, apiKeysDb, credentialsDb, notificationPreferencesDb, pushSubscriptionsDb, sessionNamesDb, applyCustomSessionNames, appConfigDb, githubTokensDb, // Backward compatibility
deviceDb, pairCodeDb };
