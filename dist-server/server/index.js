...
// node-pty is lazy-loaded to avoid crash when native module is unavailable
...
import { handleDeviceConnection, handleSignalConnection } from './modules/signal.js';
...
app.locals.wss = wss;
// In-memory maps for live device and mobile signal sessions
app.locals.deviceSockets = new Map(); // deviceId -> WebSocket
app.locals.mobileSessions = new Map(); // sessionId -> WebSocket
...
// Device management API Routes (protected)
...
// Mobile pairing page — served at /mobile so root / still serves the React SPA locally
...
wss.on('connection', (ws, request) => {
    ...
    else if (pathname === '/ws/signal') {
        ws.app = app;
        request.app = app;
        handleSignalConnection(ws, request);
    }
    else {
...
app.get('*', (req, res) => {
    ...
    if (fs.existsSync(indexPath)) {
        ...
        // No dist/ built — redirect to Vite on localhost, 404 on Render
        const host = req.hostname || '';
        if (host === 'localhost' || host === '127.0.0.1') {
...
async function startServer() {
    try {
        ...
        server.listen(SERVER_PORT, HOST, async () => {
            ...
            console.log('');
            // Keep-alive self-ping for free hosting (e.g. Render) that sleeps after inactivity
            if (process.env.RENDER_EXTERNAL_URL) {
                ...
                console.log(`${c.info('[INFO]')} Self-ping enabled → ${pingUrl}`);
            }
