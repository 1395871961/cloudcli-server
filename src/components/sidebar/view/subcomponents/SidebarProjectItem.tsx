...
const getProjectDisplayName = (project: Project): string => {
  if (!project.fullPath) return project.displayName;
  const basename = project.fullPath.replace(/\\/g, '/').split('/').filter(Boolean).pop() ?? '';
  if (basename && project.displayName === project.fullPath) return basename;
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <div className="flex min-w-0 flex-1 items-center justify-between">
                        <h3 className="truncate text-sm font-medium text-foreground">{getProjectDisplayName(project)}</h3>
                        {tasksEnabled && (
                          <TaskIndicator
                            status={taskStatus}
                            size="xs"
          ...
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0 flex-1 text-left">
              {isEditing ? (
                ...
                <div>
                  <div className="truncate text-sm font-semibold text-foreground" title={project.fullPath}>
